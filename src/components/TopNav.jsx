...
const TopNav = () => {
  ...
  const onKeyPress = e => {
    //Restrict numbers and special characters from search
    const re = /^[a-zA-Z\s]*$/; 
    if (!re.test(e.key)) {
      e.preventDefault();
    }
  ...
  const onKeyUp = e => {
    //allow user to search by city by pressing enter key
    if (e.keyCode === 13) {
      handleCitySearch();
    }
  }
  ...
  const handleCitySearch = () => {
    history.push(`/${searchInput}/imperial`)
  };
        ...
        <div className="flex items-center justify-start h-16">
          ...
          <div className="flex-1 px-2 flex justify-center lg:ml-6">
            ...
            <span className="inline-flex rounded-md shadow-sm ml-2">
              <button 
                type="button" 
                className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-gray-700 bg-white hover:text-indigo-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:text-gray-800 active:bg-gray-50 transition ease-in-out duration-150"
